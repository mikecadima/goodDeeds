...
export default function CompletedDeeds() {
	...
	async function getDeeds() {
		const response = await fetch("https://gooddeeds-server.herokuapp.com/upcoming_deeds?" + new URLSearchParams({
			status: 'completed',
	...
	return (
		<section>
			<div className="container">
				...
				{/* goodDeed Cards */}
				<div container classname="columns is-gapless is-multiline is-mobile">
					<div className="columns is-one-quarter">
						{/* card #1 start */}
						{deeds.map((deed, idx) => (
							<div className="card column is-one-quarter" key={idx}>
								<div className="card">
